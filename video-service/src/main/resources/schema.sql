-- video-service 自动建表脚本 (Spring Boot SQL 初始化, createDatabaseIfNotExist 已由 JDBC URL 处理)
...
-- 兼容已存在表: 补 tags 列 (若已存在则报错被 continue-on-error 忽略)
ALTER TABLE `video` ADD COLUMN `tags` varchar(500) DEFAULT NULL COMMENT 'Video tags' AFTER `category_id`;
