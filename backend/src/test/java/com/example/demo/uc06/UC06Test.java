...
public class UC06Test {
    ...
    @Sql(statements = {
            ...
            "INSERT INTO video (id, title, description, cover_url, play_url, video_url, user_id, category_id, duration, status, play_count, like_count, favorite_count, comment_count) VALUES (1, '测试视频', '用于弹幕测试', 'http://localhost/cover.jpg', 'http://localhost/video.mp4', 'http://localhost/video.mp4', 1, 0, 60, 'public', 0, 0, 0, 0) ON DUPLICATE KEY UPDATE play_count=VALUES(play_count), like_count=VALUES(like_count), video_url=VALUES(video_url)"
    })
    ...
    class IntegrationTest {
        ...
        void sendDanmaku_视频存在_发送成功() throws Exception {
            mockMvc.perform(post("/api/videos/1/danmakus")
                    ...
                    .andExpect(jsonPath("$.message").value("弹幕发送成功"));
        }
