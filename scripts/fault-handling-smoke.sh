...
: "${FAULT_SERVICE_URL:=http://127.0.0.1:18080/api/live/srs/health}"
: "${FAULT_OUTPUT_DIR:=${RUNNER_TEMP:-/tmp}/hangyin-fault}"
: "${FAULT_REQUEST_TIMEOUT_SECONDS:=8}"
: "${FAULT_POLL_SECONDS:=45}"

mkdir -p "${FAULT_OUTPUT_DIR}"
rm -f "${FAULT_OUTPUT_DIR}"/*
...
record_health() {
  ...
  local start end duration body curl_ok observed passed timestamp
  start="$(date +%s%3N)"
  curl_ok=false
  body=''
  if body="$(curl -fsS --connect-timeout 2 --max-time "${FAULT_REQUEST_TIMEOUT_SECONDS}" \
...
poll_health() {
  ...
  echo "Timed out waiting for ${scenario} health=${expected}" >&2
  tail -n 20 "${RESULTS}" >&2
  return 1
}

rollout() {
  kubectl -n "${KUBE_NAMESPACE}" rollout status deployment/"$1" --timeout=180s
}
...
echo 'Fault handling experiment: SRS outage, timeout fallback, recovery, and Pod self-healing'
...
# health probe has a bounded timeout instead of hanging on a dependency.
