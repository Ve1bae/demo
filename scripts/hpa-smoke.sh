...
: "${HPA_SERVICE_URL:=http://127.0.0.1:18081/api/live/rooms?page=1&pageSize=10}"
: "${HPA_OUTPUT_DIR:=${RUNNER_TEMP:-/tmp}/hangyin-hpa/live-service}"
: "${HPA_WORKERS:=32}"
: "${HPA_LOAD_SECONDS:=75}"
: "${HPA_SCALE_WAIT_SECONDS:=180}"
...
wait_metrics() {
  ...
  echo 'Metrics Server did not provide live-service metrics in time' >&2
  return 1
}
...
summarize() {
  local phase="$1" total successes errors avg p95 throughput error_rate
