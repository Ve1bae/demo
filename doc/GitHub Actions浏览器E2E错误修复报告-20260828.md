...
`主播通过页面创建直播间并看到推流信息` 首次在等待 `.live-modal` 时失败，重试通过。根因是 `/live` 路由尚未完成同步时，测试已点击顶部按钮；按钮仍处于首页状态。
...
4. 浏览器 fixture 中已创建用户 `1`、`930001-930003`，并清理重复执行产生的评论、弹幕和视频关系数据。
...
UC07 在点击前要求按钮文本为 `开始直播`，确保页面已经完成 `/live` 路由同步。
